...
## Environment variables
  ...
  DISPUTE_THRESHOLD     -- Operating threshold (default: 0.5)
...
def _run_golden_input_test(pipeline: Pipeline) -> tuple[bool, str]:
    ...
    Catches training-serving skew introduced by wrong model version,
    preprocessing mismatch, or column name changes.
    """
    try:
        ...
        if np.isnan(prob_positive):
            return False, "Golden input returned NaN -- preprocessing skew detected."
...
def _load_reference_data(path: str) -> pd.DataFrame | None:
    ...
    Samples 5,000 rows to keep memory footprint lightweight.
    ...
    try:
        ...
        logger.warning(
            "Failed to load reference data: %s. Drift detection disabled.", exc
        )
        return None
...
async def lifespan(app: FastAPI):
    ...
    _state.golden_input_passed, _state.golden_failure_message = _run_golden_input_test(
        _state.model
    )
...
def health_check() -> HealthResponse:
    """
    Health check. Re-runs golden input test on every call.

    Returns status "healthy" | "degraded" | "unhealthy".
    A "degraded" status means the model loaded but golden input parity
    is not confirmed -- predictions are still served but under warning.
    """
    if _state.model is None:
...
def predict(complaint: ComplaintInput) -> PredictionResponse:
    ...
    Complaints above the threshold are routed to the senior dispute review queue.
    """
...
def check_drift(request: DriftRequest) -> DriftResponse:
    ...
    using PSI + Chi-squared. Minimum 50 records recommended.
    """
